...
const isE2ETestMode = process.env.RCLAW_E2E_TEST === '1';
const skipSingleInstanceLock = isE2ETestMode || process.env.RCLAW_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[Rclaw] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[Rclaw] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[Rclaw] Creating main window...');
  ...
  console.log(`[Rclaw] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[Rclaw] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[Rclaw] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[Rclaw] Renderer did-finish-load');
      showWindow();
    });
  ...
  const disableAutoUpdater =
    process.env.RCLAW_DISABLE_AUTO_UPDATE === '1' || process.env.RCLAW_E2E_TEST === '1' || isCiRuntime;
  if (!disableAutoUpdater) {
    ...
    console.log('[Rclaw] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[Rclaw] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[Rclaw] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[Rclaw] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[Rclaw] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[Rclaw] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[Rclaw] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[Rclaw] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[Rclaw] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[Rclaw] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[Rclaw] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[Rclaw] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[Rclaw] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[Rclaw:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[Rclaw] Failed to restore zoom factor:', error);
...
// Register rclaw:// as the default protocol client
...
// macOS: handle rclaw:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[Rclaw] App initialization failed:', error);
    app.quit();
  });
...
app.on('before-quit', async () => {
  ...
  const timeout = new Promise<void>((resolve) => {
    setTimeout(() => {
      console.warn('[Rclaw] Cleanup timed out after 10s, forcing quit');
      resolve();
    }, 10000);
  });
...
app.on('will-quit', () => {
  console.log('[Rclaw] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[Rclaw] quit (exitCode=${exitCode})`);
});
