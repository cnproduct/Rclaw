...
export const CSRF_COOKIE_NAME = 'rclaw-csrf-token';
 ...
 *
 * 信号来源（按优先级）：
 * 1. RCLAW_HTTPS=true 或 NODE_ENV=production + HTTPS=true（显式开关）
 ...
 *
 * Signals (by priority):
 * 1. RCLAW_HTTPS=true / NODE_ENV=production + HTTPS=true (explicit opt-in)
...
function detectHttps(req?: Request): boolean {
  if (process.env.RCLAW_HTTPS === 'true' || (process.env.NODE_ENV === 'production' && process.env.HTTPS === 'true')) {
    return true;
  }
