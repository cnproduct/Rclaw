...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.RCLAW_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.RCLAW_ALLOW_REMOTE || process.env.RCLAW_REMOTE);
  const hostHint = process.env.RCLAW_HOST?.trim();
