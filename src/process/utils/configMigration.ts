...
export function getElectronConfigCandidatePaths(): string[] {
  ...
  if (process.platform === 'darwin') {
    return [
      path.join(home, '.rclaw-config', 'rclaw-config.txt'),
      path.join(home, '.rclaw-config-dev', 'rclaw-config.txt'),
    ];
  }
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(home, 'AppData', 'Roaming');
    return [
      path.join(appData, 'Rclaw', 'config', 'rclaw-config.txt'),
      path.join(appData, 'Rclaw-Dev', 'config', 'rclaw-config.txt'),
    ];
  }
  // Linux and other platforms
  return [
    path.join(home, '.config', 'Rclaw', 'config', 'rclaw-config.txt'),
    path.join(home, '.config', 'Rclaw-Dev', 'config', 'rclaw-config.txt'),
  ];
}
...
export async function migrateFromElectronConfig(configStore: ConfigStore): Promise<void> {
  try {
    ...
    if (Object.keys(sourceData).length === 0) {
      console.warn('[Rclaw] Config migration: source file appears empty or corrupted, will retry next startup');
      return;
    }
    ...
    console.log('[Rclaw] Config migrated from Electron desktop config:', sourcePath);
  } catch (error) {
    console.warn('[Rclaw] Config migration from Electron failed:', error);
  }
}
 ...
 * @param sourcePath - absolute path to an rclaw-config.txt file
...
export async function importConfigFromFile(
  ...
  try {
    ...
    if (!path.isAbsolute(sourcePath)) {
      ...
      console.warn('[Rclaw] IMPORT_CONFIG_FROM: relative path provided, resolving to:', resolved);
      sourcePath = resolved;
    }

    const sourceData = decodeConfigFile(sourcePath);
    if (Object.keys(sourceData).length === 0) {
      console.warn('[Rclaw] IMPORT_CONFIG_FROM: file is missing, empty, or corrupted:', sourcePath);
      return;
    }
    ...
    console.log('[Rclaw] Config imported from:', sourcePath, '(overwrite:', overwrite, ')');
  } catch (error) {
    console.warn('[Rclaw] IMPORT_CONFIG_FROM failed:', error);
  }
}
