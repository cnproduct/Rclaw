...
export async function resetPasswordCLI(username: string): Promise<void> {
  try {
    ...
    const dbPath = path.join(getDataPath(), 'rclaw.db');
    ...
    if (!hasUsersResult.data) {
      ...
      log.info('');
      log.info('Please run Rclaw at least once to initialize the database:');
      log.info('  rclaw --webui');
      log.info('');
      log.info('Then you can reset the password using:');
      log.info('  rclaw --resetpass <username>');
      process.exit(1);
    }
