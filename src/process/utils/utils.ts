...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'rclaw');
};
 ...
 * Release builds use ~/.rclaw; dev builds use ~/.rclaw-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.rclaw，Dev 模式使用 ~/.rclaw-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'rclaw');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.rclaw'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.rclaw-config; dev builds use ~/.rclaw-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.rclaw-config，Dev 模式使用 ~/.rclaw-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.rclaw-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[Rclaw] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[Rclaw] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[Rclaw] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[Rclaw] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
