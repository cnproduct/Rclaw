# Rclaw Database System

本文档介绍 Rclaw 的新数据库系统，它使用 **better-sqlite3** (主进程) 作为持久化存储。
...
- **数据库文件**: `{userData}/config/rclaw.db`
- **图片文件**: `{userData}/data/images/`

其中 `{userData}` 为：

- macOS: `~/Library/Application Support/Rclaw/`
- Windows: `%APPDATA%/Rclaw/`
- Linux: `~/.config/Rclaw/`
...
直接复制 `rclaw.db` 和 `rclaw.db-wal` 文件即可。
