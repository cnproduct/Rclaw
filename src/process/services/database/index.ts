...
/**
 * Main database class for Rclaw
 * Uses a pluggable ISqliteDriver for SQLite operations
 */
export class RclawDatabase {
  ...
  /**
   * Create a new RclawDatabase instance with corruption recovery.
   * This is the only way to obtain an instance — the constructor is private.
   */
  static async create(dbPath: string): Promise<RclawDatabase> {
    ...
    try {
      ...
      failedDriver = driver;
      const instance = new RclawDatabase(driver);
      instance.initialize();
      return instance;
    } catch (error) {
   ...
   * Prefer using dedicated methods on RclawDatabase where possible.
   */
...
let dbInstancePromise: Promise<RclawDatabase> | null = null;
// Synchronous reference to the resolved instance — used for safe close on exit
let dbResolved: RclawDatabase | null = null;

function resolveDbPath(): string {
  return path.join(getDataPath(), 'rclaw.db');
}

export function getDatabase(): Promise<RclawDatabase> {
  if (!dbInstancePromise) {
    dbInstancePromise = RclawDatabase.create(resolveDbPath()).then((db) => {
      dbResolved = db;
      return db;
    });
  }
