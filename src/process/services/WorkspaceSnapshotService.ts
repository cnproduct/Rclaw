...
export class WorkspaceSnapshotService {
  ...
  /**
   * Remove leftover `rclaw-snapshot-*` directories from previous sessions
  ...
  static async cleanupStaleSnapshots(): Promise<void> {
    ...
    const stale = entries.filter((name) => name.startsWith('rclaw-snapshot-'));
  ...
  private async createWorkingTreeSnapshot(workspacePath: string): Promise<string> {
    const gitdir = path.join(os.tmpdir(), `rclaw-snapshot-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    ...
    await execFileAsync(
      ...
      [
        ...
        'user.name=Rclaw',
        '-c',
        'user.email=snapshot@rclaw.local',
        'commit',
        '--allow-empty',
        '-m',
        'baseline',
