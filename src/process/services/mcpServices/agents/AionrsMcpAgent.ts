 ...
 * Maps to Rclaw transport types (snake_case)
 */
...
/**
 * Map aionrs transport type (kebab-case) to Rclaw transport type
 */
function toRclawTransportType(aionrsType: AionrsTransportType): IMcpServerTransport['type'] {
  if (aionrsType === 'streamable-http') return 'streamable_http';
  return aionrsType;
}

/**
 * Map Rclaw transport type to aionrs transport type (kebab-case)
 */
...
/**
 * Convert an aionrs server config entry to an Rclaw IMcpServer
 */
function toMcpServer(name: string, config: AionrsServerConfig): IMcpServer {
  const transportType = toRclawTransportType(config.transport);
  const now = Date.now();
...
/**
 * Convert an Rclaw IMcpServer to an aionrs server config entry
 */
...
export class AionrsMcpAgent extends AbstractMcpAgent {
  ...
  getSupportedTransports(): string[] {
    // aionrs supports stdio, sse, streamable-http (mapped to streamable_http in Rclaw)
