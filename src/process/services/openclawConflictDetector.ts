 ...
 * as Rclaw Channels, and warns the user about the conflict.
 */
...
/**
 * Check if OpenClaw Lark channel conflicts with Rclaw credentials
 */
export function detectLarkConflict(rclawAppId: string): ConflictInfo | null {
  ...
  for (const [accountName, account] of Object.entries(feishu.accounts)) {
    if (account.enabled && account.appId === rclawAppId) {
      console.warn(
        `[OpenClawConflictDetector] Lark conflict detected: OpenClaw account "${accountName}" uses same appId: ${rclawAppId}`
      );
      return {
        platform: 'lark',
        openclawEnabled: true,
        credentialMatch: true,
        openclawCredential: rclawAppId,
      };
    }
  }
...
/**
 * Check if OpenClaw Telegram channel conflicts with Rclaw credentials
 */
export function detectTelegramConflict(rclawBotToken: string): ConflictInfo | null {
  const config = readOpenClawConfig();
...
export function getConflictResolutionSteps(platform: 'lark' | 'telegram'): string[] {
  ...
  return [
    `Detected conflict: OpenClaw ${platformName} channel is using the same credentials as Rclaw.`,
    ``,
    `This means messages are being handled by OpenClaw, not Rclaw Channels.`,
    `Switching agents in Rclaw will have no effect.`,
    ``,
