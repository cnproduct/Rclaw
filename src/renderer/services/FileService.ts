...
import { RCLAW_TIMESTAMP_REGEX } from '@/common/config/constants';

// 清理Rclaw时间戳后缀，返回原始文件名
export function cleanRclawTimestamp(fileName: string): string {
  return fileName.replace(RCLAW_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(filePath: string): string {
  const fileName = filePath.split(/[\\/]/).pop() || '';
  return cleanRclawTimestamp(fileName);
}
