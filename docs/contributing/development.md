...
git clone https://github.com/iOfficeAI/Rclaw.git
cd Rclaw
...
When you have two clones of the repository (e.g. `Rclaw` and `Rclaw-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `RCLAW_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`Rclaw-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.rclaw-dev-2`, `~/.rclaw-config-dev-2`)
...
Rclaw uses **electron-vite** for fast bundling:
