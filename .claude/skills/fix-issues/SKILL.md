...
```
~/.rclaw-fix-issues/skip-list.json
```
...
```bash
gh api repos/iOfficeAI/Rclaw/issues/<number>/comments \
...
```bash
gh issue list --repo iOfficeAI/Rclaw --state open --label bug --limit 50 \
...
gh api repos/iOfficeAI/Rclaw/issues/<number>/timeline --jq \
...
```bash
gh issue comment <number> --repo iOfficeAI/Rclaw --body "$(cat <<'EOF'
...
```bash
gh pr list --repo iOfficeAI/Rclaw --state open --search "<error-keyword-or-file>" --json number,title
```
...
In daemon mode (`limit > 0`), after the summary report, update `~/.rclaw-fix-issues/skip-list.json`
...
| --------- | ---------------- | ------------------------------------------------------------------- |
| repo      | iOfficeAI/Rclaw | GitHub repository (owner/repo)                                      |
