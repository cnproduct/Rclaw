...
WORKTREE_DIR="/tmp/rclaw-ship-${PR_NUMBER}"
...
- **Worktree path** — always `/tmp/rclaw-ship-<PR_NUMBER>` (distinct from pr-automation's `/tmp/rclaw-pr-*` and pr-verify's `/tmp/rclaw-verify-*`)
