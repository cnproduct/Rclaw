...
**Worktree path:** always `/tmp/rclaw-verify-<PR_NUMBER>` — never use `/tmp/rclaw-pr-*` (reserved for pr-automation/pr-fix).
...
WORKTREE_DIR="/tmp/rclaw-verify-${PR_NUMBER}"
...
Create worktree at `/tmp/rclaw-verify-<PR_NUMBER>` (reuse if already created in Step 2):
...
在另一个终端中运行以下命令启动应用进行验证：
  ...
  cd /tmp/rclaw-verify-<PR_NUMBER> && bun run start
...
```bash
for dir in /tmp/rclaw-verify-*; do
...
```
 ...
 4. Impact analysis + test supplementation:
    a. Create worktree at /tmp/rclaw-verify-<PR> (reuse if from Step 2)
 ...
 7. Cleanup all /tmp/rclaw-verify-* worktrees → session summary (merged/auto-merge/rejected/skipped)
```
