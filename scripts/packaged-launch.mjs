...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('Rclaw.exe');
    await killProcessByName('Rclaw');
  ...
  const env = {
    ...process.env,
    RCLAW_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] RCLAW_EXTENSIONS_PATH: ${env.RCLAW_EXTENSIONS_PATH}`);
