 ...
 * parsing the electron-log file for [Rclaw:ready] / [Rclaw:init] /
 * [Rclaw:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  // Parsed from [Rclaw:init] marks
  initTotalMs: number;
  // Parsed from [Rclaw:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'Rclaw-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'Rclaw', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'Rclaw', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'Rclaw', 'logs', `${today}.log`));
  }
...
// Matches: [Rclaw:ready] <label> +<ms>ms
// Matches: [Rclaw:init]  <label> +<ms>ms
// Matches: [Rclaw:process] <label> +<ms>ms
const MARK_REGEX = /\[Rclaw:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[Rclaw] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[Rclaw] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[Rclaw] Showing main window')) marks.logs.showingMainWindow = true;
  }
...
async function launchApp(timeoutMs: number, withMemory: boolean): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      RCLAW_DISABLE_AUTO_UPDATE: '1',
      RCLAW_E2E_TEST: '1',
      RCLAW_DISABLE_DEVTOOLS: '1',
      RCLAW_CDP_PORT: '0',
      NODE_ENV: 'production',
    },
    timeout: timeoutMs,
