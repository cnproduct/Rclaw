...
/**
 * Simplified build script for Rclaw
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('Rclaw.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running Rclaw/Electron process. Attempting to close...');
        killWindowsProcesses(['Rclaw.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running Rclaw/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'Rclaw.exe');
    ...
    console.log('⚠️  Windows local build failed after Rclaw.exe was produced.');
    if (firstError) {
