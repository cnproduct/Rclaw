...
describe('ElectronPlatformServices.paths.getLogsDir', () => {
  ...
  it('returns app.getPath("logs") when it succeeds', async () => {
    mockGetPath.mockImplementation((name: string) => {
      if (name === 'logs') return '/Users/test/Library/Logs/Rclaw';
      if (name === 'userData') return '/Users/test/Library/Application Support/Rclaw';
      return `/mock/${name}`;
    });
    ...
    expect(svc.paths.getLogsDir()).toBe('/Users/test/Library/Logs/Rclaw');
  });

  it('falls back to userData/logs when app.getPath("logs") throws', async () => {
    const userData = '/Users/test/Library/Application Support/Rclaw';
    mockGetPath.mockImplementation((name: string) => {
