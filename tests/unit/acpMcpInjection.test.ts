...
describe('Step 6: loadBuiltinSessionMcpServers — builds servers list', () => {
  ...
  it('returns [teamServer] when teamMcpStdioConfig is set', async () => {
    ...
    expect(servers[0]).toMatchObject({ name: 'rclaw-team-abc', command: 'node' });
...
describe('Step 7a: createOrResumeSession — Codex vs non-Codex routing', () => {
  ...
  it('non-Codex resume: newSession options contain mcpServers array', async () => {
    ...
    expect(opts.mcpServers[0]).toMatchObject({ name: 'rclaw-team-abc' });
  });
...
describe('Step 7b PROOF-OF-FIX: Codex loadSession receives mcpServers (Task #1)', () => {
  ...
  it('PROOF-OF-FIX: loadSession receives team MCP server when teamMcpStdioConfig is set', async () => {
    ...
    // On fixed:   args[2] = [{name:'rclaw-team-abc',...}] (direct array) → PASSES
    expect(args[2]).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'rclaw-team-abc', command: 'node' })])
    );
...
describe('Step 8: Task #3 IPC mcpStatus events', () => {
  ...
  it('emits degraded when team config is missing but mcpServers ends up empty', async () => {
    ...
    const agent = createCodexAgent({
      teamMcpStdioConfig: { name: 'rclaw-team-abc', command: '', args: [], env: [] },
    });
    await callCreateOrResume(agent);
  ...
  it('event payload includes teamId derived from server name', async () => {
    ...
    // teamId extracted from 'rclaw-team-abc' → 'abc'
...
//
// How to verify "frontend can see MCP injection status" in a real Rclaw env:
//
// HAPPY PATH:
//   1. Launch Rclaw, open a Team with at least one ACP (claude/codex) agent
...
//
// DIAGNOSIS COMMANDS:
//   grep "Injecting team MCP server" ~/Library/Logs/Rclaw/*.log
//   sqlite3 ~/Library/.../rclaw.db "SELECT extra FROM conversations WHERE id='<agentConvId>'"
