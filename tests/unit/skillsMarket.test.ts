 ...
 *
 * Tests the enable/disable flow for the rclaw-skills builtin skill:
...
const BUNDLED_SKILL_PATH = path.resolve(
  __dirname,
  '../../src/process/resources/skills/_builtin/rclaw-skills/SKILL.md'
);

// Mock Electron app and initStorage before importing AcpSkillManager
vi.mock('electron', () => ({ app: { setName: vi.fn(), getPath: () => '/tmp/rclaw-test' } }));
vi.mock('../../src/process/utils/initStorage', () => ({
  getSkillsDir: () => path.join('/tmp/rclaw-test', 'skills'),
  getAutoSkillsDir: () => path.join('/tmp/rclaw-test', 'skills', '_builtin'),
  getBuiltinSkillsCopyDir: () => path.join('/tmp/rclaw-test', 'builtin-skills'),
}));
...
describe('Skills Market - Bundled SKILL.md', () => {
  ...
  it('has valid frontmatter with name and description', async () => {
    ...
    expect(nameMatch![1].trim()).toBe('rclaw-skills');

    const descMatch = frontmatter.match(/^description:\s*(.+)$/m);
    expect(descMatch).not.toBeNull();
    expect(descMatch![1]).toContain('Rclaw Skills');
  });
  ...
  it('references the standard credentials path', async () => {
    const content = await fs.readFile(BUNDLED_SKILL_PATH, 'utf-8');
    expect(content).toContain('~/.config/rclaw-skills');
  });
  ...
  it('does NOT contain the full API documentation', async () => {
    ...
    expect(content).not.toContain('X-Rclaw-Skills-Checksum');
  });
});
...
describe('Skills Market - Enable/Disable flow', () => {
  ...
  it('enable: creates rclaw-skills directory with SKILL.md', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'rclaw-skills');
    ...
    expect(written).toContain('name: rclaw-skills');
  });

  it('disable: removes rclaw-skills directory completely', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'rclaw-skills');
  ...
  it('disable: fs.rm with force does not throw if directory does not exist', async () => {
    const skillDir = path.join(tmpDir, '_builtin', 'rclaw-skills');
