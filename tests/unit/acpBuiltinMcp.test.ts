    ...
    const servers: IMcpServer[] = [
      {
        ...
        transport: {
          ...
          env: {
            RCLAW_IMG_PLATFORM: 'openai',
            RCLAW_IMG_MODEL: 'gpt-image-1',
          },
        },
        createdAt: 1,
        updatedAt: 1,
    ...
    expect(result).toEqual([
      {
        ...
        args: ['/abs/builtin-mcp-image-gen.js'],
        env: [
          { name: 'RCLAW_IMG_PLATFORM', value: 'openai' },
          { name: 'RCLAW_IMG_MODEL', value: 'gpt-image-1' },
        ],
      },
      {
        type: 'http',
...
describe('McpService Gemini detection', () => {
  ...
  it('reports built-in Gemini MCP servers under gemini source', async () => {
    ...
    vi.doMock('../../src/process/services/mcpServices/agents/RclawMcpAgent', () => ({
      RclawMcpAgent: makeAgentClass(builtinDetect),
    }));
...
describe('McpService OpenCode detection', () => {
  ...
  it('reports OpenCode MCP servers under opencode source', async () => {
    ...
    vi.doMock('../../src/process/services/mcpServices/agents/RclawMcpAgent', () => ({
      RclawMcpAgent: makeAgentClass(emptyDetect),
    }));
