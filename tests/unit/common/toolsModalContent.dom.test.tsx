...
const testState = vi.hoisted(() => ({
  ...
  initialBuiltinServer: (): MockServer => ({
    ...
    transport: {
      ...
      env: {
        RCLAW_IMG_PLATFORM: 'new-api',
        RCLAW_IMG_BASE_URL: 'https://example.com',
        RCLAW_IMG_API_KEY: 'key',
        RCLAW_IMG_MODEL: 'grok-imagine-1.0',
      },
    },
  }),
  mockConfigGet: vi.fn(),
...
describe('ToolsModalContent image generation status refresh', () => {
  ...
  it('refreshes image generation agent status only after sync completes when enabling the builtin MCP server', async () => {
    ...
    expect(testState.mockCheckSingleServerInstallStatus).toHaveBeenCalledWith('rclaw-image-generation');
  });
