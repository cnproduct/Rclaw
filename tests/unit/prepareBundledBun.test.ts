...
describe('prepareBundledBun', () => {
  ...
  const originalCacheDir = process.env.RCLAW_BUN_CACHE_DIR;
  const originalVersion = process.env.RCLAW_BUN_VERSION;
  ...
  afterEach(() => {
    process.env.RCLAW_BUN_CACHE_DIR = originalCacheDir;
    process.env.RCLAW_BUN_VERSION = originalVersion;
  ...
  function setupCacheAndBackup(version: string) {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'rclaw-bun-test-'));
    ...
    process.env.RCLAW_BUN_CACHE_DIR = cacheRoot;
    process.env.RCLAW_BUN_VERSION = version;
