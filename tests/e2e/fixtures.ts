 ...
 *      (e.g. out/linux-unpacked/rclaw, out/mac-arm64/Rclaw.app, out/win-unpacked/Rclaw.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rclaw-e2e-state-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/Rclaw.exe  or  out/win-x64-unpacked/Rclaw.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'Rclaw.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/Rclaw.app/Contents/MacOS/Rclaw  or  out/mac/Rclaw.app/...
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    RCLAW_EXTENSIONS_PATH: process.env.RCLAW_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    RCLAW_EXTENSION_STATES_FILE: process.env.RCLAW_EXTENSION_STATES_FILE || e2eStateFile,
